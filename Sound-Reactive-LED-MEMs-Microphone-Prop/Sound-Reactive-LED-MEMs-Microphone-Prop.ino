...
#define LED_TOTAL 10  //Change this to the number of LEDs in your strand.
...
const int sampleWindow = 30; // Sample window width in mS (250 mS = 4Hz)
int knock;
...
void loop() {  //This is where the magic happens. This loop produces each frame of the visual.
  ...
  // electret mic calculation from example code
  // collect data for t  miliseconds
  while (millis() - start < sampleWindow)
  {
    knock = analogRead(AUDIO_PIN);
  ...
  //there is an offset voltage since this code is using an eletret mic instead of the envelope
  //the value works for this application
  if (peakToPeak - 40 < 0 )
  {
    peakToPeak = 0; //offset the signal to about 0 if below certain value
  }
  else {
  ...
  if (peakToPeak > 255)
  {
    volume = 255;//if it gets too loud then it gets cut off, so lets just make it 255 when volume hits the rail. If we use the map() function it won't pick up the sound as easily
  }
  else
  {
    volume = peakToPeak;
  ...
  //Map the mic peak-to-peak amplitude to a volume between 0 and 255 because of different data types
  //volume = map(peakToPeak, 0, 983, 0, 255);
  ...
  delay(10);     //Paces visuals so they aren't too fast to be enjoyable, adjusted from 30
}
//////////</Standard Functions>
